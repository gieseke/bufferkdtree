...
__kernel void do_bruteforce_all_leaves_nearest_neighborsFab(
...
__kernel void do_bruteforce_all_leaves_nearest_neighbors(
    ...
    FLOAT_TYPE cur_max_dist = MIN_FLOAT_TYPE;
    uint       cur_max_ind  = 0;

    // initialize private copies (distances and indices)    
    if (!all_brute){
        for (j=0; j<K_NN; j++){
            FLOAT_TYPE tmp_dist = dist_mins_global_tmp[j*total_number_test_indices_removed + tid];
            uint       tmp_ind  =  idx_mins_global_tmp[j*total_number_test_indices_removed + tid];
            uint    cond = (tmp_dist > cur_max_dist);
            cur_max_dist = cond*tmp_dist + (1-cond)*cur_max_dist;
            cur_max_ind  = cond*j        + (1-cond)*cur_max_ind;
            dist_mins_private[j] = tmp_dist;
             idx_mins_private[j] =  tmp_ind; 
        }
    } else { // for final brute-force: use fresh distances!
        for (j=0; j<K_NN; j++){
            dist_mins_private[j] = MAX_FLOAT_TYPE;
    ...
    for (train_idx=fr_idx; train_idx<to_idx; train_idx++){
        ...
        for (j=0; j<dim_mul_vecsize; j+=VECSIZE){
            ...
            tmp_vec = tmp_vec - test_patt_vec[j/VECSIZE];
#else
            FLOAT_TYPE4 test_vct = (FLOAT_TYPE4)(
                   test_patterns_subset[(j+0)*total_number_test_indices_removed + tid],
                   test_patterns_subset[(j+1)*total_number_test_indices_removed + tid], 
                   test_patterns_subset[(j+2)*total_number_test_indices_removed + tid],
                   test_patterns_subset[(j+3)*total_number_test_indices_removed + tid]
               );
            tmp_vec = tmp_vec - test_vct;             
#endif
            dist_tmp_vec += tmp_vec*tmp_vec;
        }
        ...
        for (j=dim_mul_vecsize; j<DIM; j++){
#if (COMPUTE_GLB_ONCE != 0)
            tmp =   (train_patterns_sorted[train_idx_offset*DIM + j] -
                    test_patt_scalar[j-dim_mul_vecsize]);
#else 
            tmp =   (train_patterns_sorted[train_idx_offset*DIM + j] -
                    test_patterns_subset[j*total_number_test_indices_removed + tid]);
#endif
            dist_tmp += tmp*tmp;
        }
        dist_tmp += dist_tmp_vec.s0 + dist_tmp_vec.s1 + dist_tmp_vec.s2 + dist_tmp_vec.s3;

        if(cur_max_dist > dist_tmp) { // cosmin: sort only at the end;
            dist_mins_private[cur_max_ind] = dist_tmp;
            idx_mins_private [cur_max_ind] = train_idx;
            
            cur_max_dist = dist_tmp;
            for(j=0; j<K_NN; j++) {
                FLOAT_TYPE this_ind_dist = dist_mins_private[j];
                uint    cond = (this_ind_dist > cur_max_dist);
                cur_max_dist = cond*this_ind_dist + (1-cond)*cur_max_dist;
                cur_max_ind  = cond*j             + (1-cond)*cur_max_ind;
            }
        }
    }    
